...
class Chart{
  constructor(el,o){this.o=o;el.innerHTML=`<div class="panel-head"><h2>${o.title}</h2><span class="spot">${fmt(o.spot,o.decimals,o.unit)}</span></div><div class="chart-wrap"><canvas></canvas><div class="legend"></div><div class="tip"></div></div>`;this.canvas=el.querySelector('canvas');this.wrap=el.querySelector('.chart-wrap');this.tip=el.querySelector('.tip');this.legend=el.querySelector('.legend');this.ctx=this.canvas.getContext('2d');
    ...
    this.now=Date.now();this.vol=o.candles?realizedVol(o.history):.01;if(o.layers){o.mode=o.mode||'both';o.horizons=mergeLayers(o.layers.pm,o.layers.opt,o.mode)}this.build();if(typeof window!=='undefined')(window.__charts=window.__charts||[]).push(this);
  ...
  seg(t,hs=this.hs){let n=0;while(n<hs.length&&hs[n].t<t)n++;if(n>=hs.length)return null;const a=n?hs[n-1]:this.spotH,b=hs[n];return{a,b,w:Math.max(0,Math.min(1,(t-a.t)/Math.max(1,b.t-a.t)))}}
  ...
  draw(){const r=this.wrap.getBoundingClientRect(),d=devicePixelRatio||1;if(!r.width)return;this.w=r.width;this.h=r.height;const cw=Math.round(r.width*d),ch=Math.round(r.height*d);if(this.canvas.width!==cw||this.canvas.height!==ch){this.canvas.width=cw;this.canvas.height=ch}this.ctx.setTransform(d,0,0,d,0,0);
    ...
    const key=[this.view.t0,this.view.t1,this.lo,this.hi,cw,ch,this.o.mode||''].join();
...
async function boot(){try{const d=await(await fetch(DATA_URL+'?t='+Date.now())).json(),now=Date.now(),p=d.panels.BTC,e=document.querySelector('#btc-panel'),pmHs=selectHorizons(p.horizons,now,d.generated_at,'BTC','pm'),optHs=selectHorizons(p.horizons,now,d.generated_at,'BTC','opt'),hs=mergeLayers(pmHs,optHs,'both'),ag=document.querySelector('#data-age');
    ...
    new Chart(e,{title:'BTC/USD',spot,unit:'USD',decimals:0,horizons:hs,layers:{pm:pmHs,opt:optHs},mode:'both',history:hist,candles:true,generatedAt:d.generated_at});
