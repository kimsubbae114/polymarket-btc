...
function dateLabel(t){const d=new Date(t);return`${d.getMonth()+1}/${d.getDate()}`}
...
class Chart{
  ...
  density(){const hs=this.o.horizons;if(!hs.length)return;const x0=Math.max(this.L,Math.ceil(this.x(this.now))),x1=Math.min(this.L+this.pw,Math.floor(this.x(this.max)));const W=x1-x0;if(W<1)return;const H=Math.ceil(this.ph),o=document.createElement('canvas');o.width=W;o.height=H;const g=o.getContext('2d'),im=g.createImageData(W,H);
    for(let i=0;i<W;i++){const t=this.tAt(x0+i),n=hs.findIndex(h=>Date.parse(h.t)>=t);if(n<0)continue;const a=n?hs[n-1]:{bins:[{lo:this.o.spot*.996,hi:this.o.spot*1.004,p:1}]},b=hs[n],ta=n?Date.parse(a.t):this.now,w=Math.max(0,Math.min(1,(t-ta)/Math.max(1,Date.parse(b.t)-ta))),ds=[];for(let j=0;j<H;j++)ds.push((1-w)*smoothDensityAt(a.bins,this.vAt(this.T+j))+w*smoothDensityAt(b.bins,this.vAt(this.T+j)));columnAlphas(ds,(a.kind==='touch-approx'||b.kind==='touch-approx')?.6:1).forEach((q,j)=>q&&im.data.set([245,165,36,Math.round(q*255*.6)],(j*W+i)*4))}
  ...
  futureDraw(){const{t0,t1}=this.view;this.future.forEach(k=>{if(k.t>=t0-DAY&&k.t<=t1+DAY)this.candle(this.x(k.t),k)})}
  marks(){// 만기마다 최빈값 점 + (간격이 넉넉할 때만) 라벨. 작은 패널은 현재값에서 점까지 얇은 점선.
    ...
    if(this.o.small){c.setLineDash([2,3]);c.strokeStyle='rgba(245,165,36,.6)';c.beginPath();c.moveTo(this.x(this.now),this.y(this.o.spot));hs.forEach((h,i)=>{const m=modeBin(h.bins);c.lineTo(xs[i],this.y((m.lo+m.hi)/2))});c.stroke();c.setLineDash([])}
